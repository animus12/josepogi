...
class Auth extends BaseController
{
    ...
    public function signup()
    {
        ...
        $this->form_validation->set_rules('social','Social Media Link','trim|required', array(
            'required' => lang('this_field_is_required')
        ));
        ...
        else
        {
            ...
            $social = strtolower($this->input->post('social', TRUE));
            $password = $this->input->post('password', TRUE);
            $ref = $this->input->post('ref', TRUE);
            $roleId = '3';
            $dateCreated = date('Y-m-d H:i:s');
            $result = $this->login_model->checkEmailExist($email); 
            $verification_code = $companyInfo['signup_email_verification'] == 0 ? NULL : time() . rand(10*45, 100*98);;
                ...
                if($result1>0)
                {
				    ...
				    if($resultEmail->num_rows() > 0)
				    {
					    ...
					    $splVars = array(
                            ...
                            "!verification_url" => base_url('verify-email/'.$verification_code),
            ...
            if($result){
                ...
                if($resultEmail->num_rows() > 0)
                {
                    ...
                    $splVars = array(
                        ...
                        "!clientEmail" => $email,
                        "!companyName" => $companyInfo['name'],
                        "!verification_url" => base_url('verify-email/'.$result->email_verification_code),
                        "!address"     => $companyInfo['address'],
                        "!siteurl"     => base_url()
                    );
    ...
    public function verify_email($verification_code)
    {
        ...
        if($row){
            ...
            $this->loadViews('/auth/verified', $this->global, NULL);
        } else {
            $this->loadThis();
        }
    }
        ...
        else
        {
            ...
            if(!empty($result))
            {
                ...
                } else
                {
                    ...
                    {
                        //Check if email_verification is active and user has verified email
                        if($companyInfo['signup_email_verification'] == 1){
                            //Check if the user has verified email
                            if($result->email_verification_code == NULL){
                                //Proceed
                                $sessionArray = array(
                                ...
                                $array = array(
                                    'success'=>false,
                                    'errors' => array(
                                        'email' => 'Please click the link on your email to verify your account!'
                                    )
                                );
    ...
    public function loginMe()
    {
        ...
        {   
            ...
            if($companyInfo['two_factor_auth'] == 'Google Authenticator')
            {
                ...
                $secret = $this->user_model->getUserInfoByEmail($email)->g_auth_secret;     
